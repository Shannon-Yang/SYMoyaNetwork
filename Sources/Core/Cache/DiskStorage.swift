    ...
    public class Backend<T: DataTransformable> {
            ...
            expiration: StorageExpiration? = nil) throws
        {
            ...
            do {
                ...
                throw SYMoyaNetworkError.cacheError(reason: .cannotConvertToData(object: value, error: error))
            }
            ...
            extendingExpiration: ExpirationExtending) throws -> T?
        {
            ...
            do {
                ...
                throw SYMoyaNetworkError.cacheError(reason: .cannotLoadDataFromDisk(url: fileURL, error: error))
            }
        }
        ...
        ///
        /// - Parameter key: The final computed key used when caching the response. Please note that usually this is not
        /// the `cacheKey` of an response `Source`. It is the computed key with processor identifier considered.
        ///
        /// - Note:
        /// This method does not guarantee there is an response already cached in the returned URL. It just gives your
        /// the URL that the response should be if it exists in disk storage, with the give key.
        ///
        ...
        func allFileURLs(for propertyKeys: [URLResourceKey]) throws -> [URL] {
            ...
            guard let urls = directoryEnumerator.allObjects as? [URL] else {
                throw SYMoyaNetworkError.cacheError(reason: .invalidFileEnumeratorContent(url: directoryURL))
            }
            return urls
        }
...
extension DiskStorage {
    ...
    public struct Config {
        ...
        /// Default is `false`
        /// If set to `true`, response extension will be extracted from original file name and append to
...
extension DiskStorage {
    struct Creation {
        ...
        init(_ config: Config) {
            ...
            cacheName = "com.shannonyang.SYMoyaNetwork.NetworkCache.\(config.name)"
