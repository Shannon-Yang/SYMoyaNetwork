...
public class SwiftyJSONResponseSerializer: ResponseSerializer {
    ...
    public func serialize(result: SYMoyaNetworkResult) -> SYMoyaNetworkDataResponse<SerializedObject> {
        result.serializerSwiftyJSON(ptions: opt)
    }
}

extension ResponseSerializer where Self == SwiftyJSONResponseSerializer {
    /// Default serializer object
    public static var defaultSwiftyJSONSerializer: SwiftyJSONResponseSerializer {
    ...
    public static func swiftyJSON(ptions opt: JSONSerialization.ReadingOptions = []) -> SwiftyJSONResponseSerializer {
        SwiftyJSONResponseSerializer(ptions: opt)
    }
}
