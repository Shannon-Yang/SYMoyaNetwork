...
extension Reactive where Base: SYMoyaProviderRequestable {
    ...
    /// - Returns: A type-erased `ObservableType` that can produce values of type `SYMoyaNetworkDataResponse<Decodable>`
    public func responseDecodableObjectFromCache<T: Decodable>(_ cacheFromType: NetworkCacheFromType = .memoryOrDisk, target: Base.Target, serializer: DecodableResponseSerializer<T> = .defaultDecodableSerializer, callbackQueue: DispatchQueue? = .none) -> Observable<SYMoyaNetworkDataResponse<T>> {
    ...
    /// - Returns: A type-erased `ObservableType` that can produce values of type `SYMoyaNetworkDataResponse<Decodable>`
    public func responseDecodableObject<T: Decodable>(_ type: ResponseDataSourceType = .server, target: Base.Target, serializer: DecodableResponseSerializer<T> = .defaultDecodableSerializer, callbackQueue: DispatchQueue? = .none, progress: ProgressBlock? = .none) -> Observable<SYMoyaNetworkDataResponse<T>> {
        Observable.create { [weak base] observer in
