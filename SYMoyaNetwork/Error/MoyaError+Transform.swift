...
public extension MoyaError {
    ...
    func transformToSYMoyaNetworkError() -> SYMoyaNetworkError {
        ...
        case .statusCode(let response):
            return .serializeError(reason: .statusCodeFail(response: response))
        /// Indicates a response failed due to an underlying `Error`.
        case .underlying(let error, let response):
            return .serializeError(reason: .underlying(error: error, response: response))
        /// Indicates that an `Endpoint` failed to map to a `URLRequest`.
        case .requestMapping(let string):
            return .serializeError(reason: .urlRequestCreateFail(string: string))
        /// Indicates that an `Endpoint` failed to encode the parameters for the `URLRequest`.
        case .parameterEncoding(let error):
            return .serializeError(reason: .parameterEncodingError(error: error))
        }
    }
